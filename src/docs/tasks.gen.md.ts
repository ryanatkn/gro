...
export const gen: Gen = async ({originId}) => {
	...
	const originDirname = dirname(originId);
	const originBasename = basename(originId);
...
[← back to docs](${relative(originDirname, toSourceId('docs')) || './'})

> generated by [\`${originBasename}\`](${originBasename})
`;
};
