...
const Args = z
	.object({
		...
		dirname: z
			...
			.optional(),
		source: z
			.string({description: 'source branch to build and commit from'})
			.default(GIT_DEPLOY_SOURCE_BRANCH),
		target: z.string({description: 'target branch to commit to'}).default(GIT_DEPLOY_TARGET_BRANCH),
		dry: z
			.boolean({
				description:
					'build and prepare to commit without actually commiting, for diagnostic and testing purposes',
			})
			.default(false),
		clean: z
			.boolean({
				description: 'instead of building and commiting, just clean the git worktree and Gro cache',
			})
			.default(false),
		force: z
...
export const task: Task<Args> = {
	...
	run: async ({fs, args, log}): Promise<void> => {
		...
		if (!force && target !== GIT_DEPLOY_TARGET_BRANCH) {
			throw Error(
				`Warning! You are commiting to a custom target branch '${target}',` +
		...
		if (gitTargetExistsResult.ok) {
			...
			// Fetch the remote target commit branch.
		...
		// Checkout the source branch to commit.
		...
		// Set up the commitment `target` branch if necessary.
		...
		try {
			...
			// After the build is ready, set the commited directory, inferring as needed.
			...
			if (!(await fs.exists(dir))) {
				log.error(red('directory to commit does not exist after building:'), dir);
				return;
			}
			...
			if (dry) {
				log.info(red('dry commit failed'));
			}
			throw Error(`Deploy safely canceled due to build failure. See the error above.`);
		}

		// At this point, `dist/` is ready to be committed and commited!
		if (dry) {
			log.info(green('dry commit complete:'), 'files are available in', printPath(dir));
			return;
		}

		try {
			// Set up the commitment worktree
			await spawn('git', ['worktree', 'add', WORKTREE_DIRNAME, target]);
			// Pull the remote commit branch, ignoring failures
		...
		log.info(rainbow('commited')); // TODO log a different message if "Everything up-to-date"
	},
};
