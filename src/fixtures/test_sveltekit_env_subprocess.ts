...
async function runTest() {
	...
	try {
		...
		const fixture = await import(resolve('src/fixtures/test_sveltekit_env.ts'));
		
		if (fixture.exported_env_static_public !== VALUE) {
			console.error(`✗ Static import test failed: expected "${VALUE}", got "${fixture.exported_env_static_public}"`);
			process.exit(1);
		}
		...
		if (!envMod.PUBLIC_SOME_PUBLIC_ENV_VAR || envMod.PUBLIC_SOME_PUBLIC_ENV_VAR !== VALUE) {
			console.error(`✗ Dynamic import test failed: expected "${VALUE}", got "${envMod.PUBLIC_SOME_PUBLIC_ENV_VAR}"`);
			process.exit(1);
		}
