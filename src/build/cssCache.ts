...
import {printKeyValue} from '@feltcoop/felt/util/print.js';
...
export const createCssCache = <T extends CssBuild = CssBuild>(): CssCache<T> => {
	...
	return {
		...
		getCssBuild: (bundleName, id) => {
			...
			if (!bundle) {
				throw Error(`Expected to find CSS bundle name '${bundleName}'`);
			}
			const cssBuild = bundle.buildsById.get(id);
			if (!cssBuild) {
				throw Error(`Expected to find CSS build with id '${id}' for '${bundleName}'`);
			}
			return cssBuild;
		},
